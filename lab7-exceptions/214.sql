...
-- uniemożliwić ponowne wstawienie zdanego egzaminu określonego studenta z danego
-- przedmiotu w sytuacji, gdy taki przedmiot został już zdany przez tego studenta. Zadanie
-- należy rozwiązać z użyciem techniki wyjątków. Jeśli wstawiany rekord narusza powyższą
-- zasadę, wówczas należy wyświetlić odpowiedni komunikat. Jeśli nie – rekord z takim
-- egzaminem należy wstawić do tabeli Egzaminy.
...
declare
    ...
    begin
        if passedExamsCount(in_id_student, in_id_przedmiot) > 0 and in_zdal = 'T' then
...
begin
    ...
    addNewExam(v_examin.ID_EGZAMIN, v_examin.ID_STUDENT, v_examin.ID_PRZEDMIOT, v_examin.ID_EGZAMINATOR, v_examin.DATA_EGZAMIN, v_examin.ID_OSRODEK, v_examin.ZDAL, v_examin.PUNKTY);
end;